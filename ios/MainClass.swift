...
//  Snowboy
//
//  Created by Mostafa Hendawi on 5/29/20.
...
@objc(MainClass)
class MainClass: NSObject, AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    ...
    @objc func helloReact() {
        wrapper = SnowboyWrapper(resources: RESOURCE, modelStr: MODEL)
        wrapper.setSensitivity("0.5")
        wrapper.setAudioGain(1.0)
        print("Sample rate: \(String(describing: wrapper?.sampleRate())); channels: \(String(describing: wrapper?.numChannels())); bits: \(String(describing: wrapper?.bitsPerSample()))")
